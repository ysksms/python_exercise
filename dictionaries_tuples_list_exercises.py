...
print(
    f"* The student with the highest average score is {highest_avg_student} with an average score of {highest_avg_score}"
)
...
print(
    f"* The student with the lowest average score is {lowest_avg_student} with an average score of {lowest_avg_score}"
)
...
for product, details in inventory.items():
    print(
        f"* {product.capitalize()}: Quantity = {details[0]}, Price per unit = ${details[1]}"
    )
...
for isbn, details in library.items():
    print(
        f"* ISBN: {isbn} - Title: {details['title']}, Author: {details['author']}, Year: {details['year']}"
    )
...
for movie, details in movies.items():
    print(
        f"* Title: {movie}, Year: {details['year']}, Rating: {details['rating']}, Genre: {details['genre']}"
    )
...
for product in cart:
    print(
        f"* Item: {product['item']}, Quantity: {product['quantity']}, Price per unit: ${product['price_per_unit']}"
    )
...
for day, details in weather.items():
    print(
        f"* {day}: Temperature = {details['temperature']}°C, Humidity = {details['humidity']}%"
    )
